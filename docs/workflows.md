...
Stopping (`x`) is two-stage: the run tells the script to abort and gives it the configured `abortGraceMs` window (5 seconds by default) to run its `finally` and optional `__onWorkflowAbort` hook before the worker is hard-terminated. The abort acknowledgement is informational and cannot shorten that deadline; a script wedged in a tight loop is terminated when the grace window expires. Either way the run settles as stopped, and the terminal state — the run-status file, plus a completion notification — is guaranteed even though the script never returned. When the run stops or errors, that completion notification also names the last agent still mid-flight (`<last-agent>`), so the main agent sees exactly which child was left working. A completed run has no un-converged agent to name, so it omits the line.

A run can also be stopped without opening the inspector: the model — or the main agent — calls `StopSubagentWorkflow` with the `Task ID:` a `SubagentWorkflow` call returned. The tool returns immediately after requesting the stop; it does not wait out the 5-second abort grace. The runtime owns what happens next: the script converges inside the grace window, the terminal run-status record is persisted, and exactly one completion notification is sent — a stopped run reads as `Stopped` (status `killed`) and names the last agent left in flight. A task id only addresses the session that started it, so the tool is current-session only. Repeated stops report that a stop is already in flight; a stop on a settled run reports its status; an unknown id is reported clearly rather than starting a new run.
...
| The run-status snapshot | the same directory, `<run id>.run-status.json` |
| Saved workflows | `.pi/workflows/` → `.agents/workflows/` → `<agent dir>/workflows/`, first hit wins |

The first three are scratch: temp storage, wiped by a reboot or a temp sweep. Only the fourth is durable, and copying a script there is a manual step.

The run-status snapshot is written the moment a run settles — completed, failed **or stopped** — so a script that is hard-killed mid-turn still leaves a record of how it ended, what the error was, and where it got to. It sits next to the journal because that is the directory a resume already reads; it is not itself read back.
...
The sandbox is a determinism and accident boundary, not a defence against a deliberately hostile script: the injected globals are host closures, and disabled code generation is what actually stops one being used to compile anything. `setTimeout` and `clearTimeout` are also available — use them in `try/finally` blocks to let the script converge within the abort grace window.
