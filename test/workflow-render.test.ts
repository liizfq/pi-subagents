...
describe("run_status entries", () => {
  // The watchdog's run_status entries are neither agents nor logs; the card must
  // tolerate them without crashing or inventing a phase group.
  const runStatus = { type: "run_status", state: "stalled", idleMs: 5 } as unknown as WorkflowEntry;

  it("does not crash the card or invent a phase group", () => {
    const lines = card({
