...
describe("run_status entries", () => {
  /**
   * The watchdog's `run_status` entries are not a phase, a log or an agent, and
   * must not be folded into any of the three. Cast through unknown until the
   * type joins the WorkflowEntry union.
   */
  const runStatus = (partial: Partial<{ state: string; idleMs: number }> = {}): WorkflowEntry =>
    ({ type: "run_status", state: "idle_warning", idleMs: 5, ...partial }) as unknown as WorkflowEntry;

  it("does not leak run_status into phaseTitles", () => {
    const { agents, logs, phaseTitles } = collapse([runStatus()]);
    expect(agents).toHaveLength(0);
    expect(logs).toHaveLength(0);
    // Today the unknown entry type lands in phaseTitles under an undefined
    // key — exactly the pollution that breaks the UI's phase grouping.
    expect([...phaseTitles.entries()]).toHaveLength(0);
  });

  it("does not treat run_status as a phase in buildPhaseGroups", () => {
    expect(buildPhaseGroups([runStatus()])).toHaveLength(0);
  });

  it("keeps real phases intact beside run_status entries", () => {
    const groups = buildPhaseGroups([
      { type: "workflow_phase", index: 0, title: "Scan" },
      runStatus(),
      agentEntry({ index: 0, phaseIndex: 0, state: "done" }),
    ]);
