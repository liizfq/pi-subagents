...
describe("detectEnv", () => {
  it("detects a git repo", async () => {
    // A fresh repo, not the workspace: detectEnv must not depend on the cwd
    // the suite happens to run from being a checkout.
    const tmpDir = mkdtempSync(join(tmpdir(), "pi-env-repo-"));
    try {
