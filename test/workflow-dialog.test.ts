...
describe("run_status entries", () => {
  // The watchdog's run_status entries are neither agents nor logs; the dialog
  // must tolerate them without crashing or polluting its phase groups.
  const runStatus = { type: "run_status", state: "stalled", idleMs: 5 } as unknown as WorkflowEntry;

  it("does not crash the dialog or invent a group", () => {
    const lines = dialog({
      progress: [runStatus, agentEntry({ index: 0, label: "review:bugs", state: "done" })],
    });
    expect(cells(lines).length).toBeGreaterThan(0);
