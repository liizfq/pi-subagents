...
/* ------------------------------------------------------------------------- *
 * P0 — abort terminal state, grace, and the watchdog
 * ------------------------------------------------------------------------- */

describe("abort terminal state", () => {
  /** A host whose child hangs until the run is aborted. */
  const hangingHost = () => stubHost(() => new Promise<WorkflowSpawnResult>(() => {}));

  it("reports the terminal summary to onTerminal and persistRunSummary on abort", async () => {
    const controller = new AbortController();
    const { host, aborted } = hangingHost();
    ...
    expect(result.error).toBe("Workflow aborted.");
    expect(aborted).toEqual(["wf-agent-0"]);
    // The runtime must hand the terminal state over on its own — a worker that
    // is hard-killed leaves no script behind to write anything.
    expect(terminals).toHaveLength(1);
    expect(terminals[0].status).toBe("killed");
    expect(terminals[0].error).toBe("Workflow aborted.");
    expect(terminals[0].agentCount).toBe(1);
  ...
  it("fires onTerminal for a completed run too", async () => {
    const { host } = stubHost();
    const terminals: WorkflowRunSummary[] = [];
    const result = await run('return await agent("a");', {
      host,
      onTerminal: summary => { terminals.push(summary); },
    });
  ...
  it("fires onTerminal for a failed run too", async () => {
    const { host } = stubHost();
    const terminals: WorkflowRunSummary[] = [];
    const result = await run('throw new Error("script blew up");', {
      host,
      onTerminal: summary => { terminals.push(summary); },
    });
    ...
    expect(terminals[0].error).toBe("script blew up");
  });

  it("lets the script catch the abort as a thrown error, not a null", async () => {
    // Today the abort resolves the awaiting agent() with null (or never
    // answers it before the hard terminate), so a script's try/catch cannot
    // run. The abort must propagate as a catchable error instead.
    const controller = new AbortController();
    ...
    expect(result.status).toBe("killed");
    const caught = result.progress.find(
      e => e.type === "workflow_log" && String(e.message).includes("caught abort"),
    );
  ...
  it("keeps the run killed even when the script converges inside the grace window", async () => {
    ...
    expect(result.status).toBe("killed");
    expect(result.progress.some(e => e.type === "workflow_log" && e.message === "converged")).toBe(true);
  });
});
