...
**Stuck detection** (`stuckDetection`, default `"rules"`): detects subagents that are stuck — repeating the identical tool call (same tool + same args), failing in a loop, idling with no output, or having no activity at all. The rule layer runs every `stuckRuleIntervalMs` (default `30000` = 30s); a window that violates marks the run `suspicious`, and after `stuckGraceWindows` (default `3`, ≈90s) consecutive suspicious windows the run is `steer`ed once ("wrap up") and then `aborted` through the existing max-turns-style path. Set `"rules+ai"` to add an AI verdict: when a run stays suspicious, a cheap model (`stuckAiModel`, default `deepseek-v4-flash`, resolved via `resolveModel`) is asked once per `stuckAiIntervalMs` (default `600000` = 10min) whether the run is genuinely stuck — a "not stuck" verdict resets it to healthy, a "stuck" verdict aborts immediately. The AI prompt carries an activity summary (tool starts/ends, text progress, turn ends) with tool arguments intentionally omitted. All six fields are settable from `/agents` → Settings and persist in `subagents.json`. An aborted run's record carries `status: "aborted"` and its `stuckState`; a workflow's `agent()` call then receives `{ ok: false, error: "The agent was aborted after sustained stuck activity." }` through the existing `toSpawnResult` path — no new workflow runtime mechanism.
