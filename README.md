...
A [pi](https://pi.dev) extension that brings **Claude Code-style autonomous sub-agents and workflow orchestration** to pi. Spawn specialized agents that run in isolated sessions — each with its own tools, system prompt, model, and thinking level. Run them in the background (the default) or block on them, steer them mid-run, resume completed sessions, and define your own custom agent types. When the orchestration shouldn't be improvised, hand a deterministic JavaScript script to the `SubagentWorkflow` tool — `agent()`, `parallel()`, `pipeline()`, `chain()`, `dag()` — and scripts written for Claude Code's `Workflow` tool run here unchanged. `chain()` is a pi-only serial primitive and `dag()` is a pi-only DAG scheduler: `dag({ nodes })` takes a map of node ids to `{ prompt, deps?, label? }`, topologically sorts the graph, runs ready nodes concurrently (bounded by the run's concurrency cap), and skips every transitive dependent of a failed node.
...
- **Scripted workflows** — a `SubagentWorkflow` tool that runs a deterministic JavaScript script orchestrating many subagents: `agent()`, `parallel()`, `pipeline()`, `chain()`, `dag()`, `phase()`, `log()` and `args`, with a pure-literal `meta` block declaring the phases. `chain(steps)` runs its steps strictly serially and short-circuits at the first failed step, unlike `pipeline()` which advances each item through stages independently. `pipeline()` has no barrier between stages, so one item can be in a later stage while another is still in the first — unlike `parallel()`, which idles every fast agent until the slowest finishes. `dag({ nodes })` is the DAG scheduler: each node is `{ prompt, deps?, label? }`, the graph is topologically sorted (a cycle fails the run), nodes with no unresolved dependencies run concurrently (bounded by the run's concurrency cap), and every transitive dependent of a failed node is marked `skipped` and never launched. Runs in the background with a live card, inspectable via `/agents → Workflows` or by selecting the run in FleetView. `agent()` also takes `gate: "npm test"` to verify a child by running a command (inside its worktree, when isolated) rather than asking another model, and `resume: "<label>"` to continue a child instead of re-paying its context. Scripts run in a `node:vm` sandbox on a worker thread where `Date.now()`, `Math.random()` and `eval` throw, while `setTimeout` and `clearTimeout` are available (host closures) for abort-convergence `try/finally` blocks. On by default, but it stands down for company: if another extension already provides a `Workflow` or `SubagentWorkflow` tool, this one warns and disables itself for the session rather than offering the model two orchestrators. Pin it either way with `"workflowsEnabled"` in `subagents.json` or `/agents → Settings → Workflows`. A script written for Claude Code's `Workflow` tool runs here unchanged: same globals, `schema` returns a validated object exactly as it does there, `budget` is present and always reports no token target (pi has no such directive) so its `budget.total`-guarded patterns still take the branch they were written for, and nested `workflow()` composes saved workflows one level deep. A stopped or failed run always lands a terminal record and a completion notification — even when the script is hard-killed mid-turn — and a run idle for ten minutes is flagged stalled rather than hanging silently. A `StopSubagentWorkflow` tool stops a running or paused workflow by the `Task ID:` a `SubagentWorkflow` call returned: current session only, immediate return, and the terminal record plus completion notification still come from the runtime after the 5-second abort grace. **[Full guide](https://github.com/tintinweb/pi-subagents/blob/master/docs/workflows.md)**
...
Run a deterministic script that orchestrates many subagents. Returns a task id immediately; the run continues in the background and notifies on completion. When a run settles as stopped or errored, that completion notification also names the last agent still in flight, so the main agent sees which child was left working; a completed run omits the line.
...
Concurrency is capped at `max(1, min(16, cpus - 2))` — the run's own limit, independent of the session's `maxConcurrent` pool, which its agents do not enter. There are 1000 agents per run and 4096 items per `parallel`/`pipeline`/`chain`/`dag` call.
...
### `StopSubagentWorkflow`

Stop a running or paused workflow in the current session.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `taskId` | string | yes | The `Task ID:` value a `SubagentWorkflow` call returned; only runs started in this session are addressable |

The tool returns immediately after requesting the stop — it does not wait for the run's 5-second abort grace. The runtime owns everything after the request: the script converges inside the grace window, the terminal run-status record is persisted, and the single completion notification is sent (a stopped run settles as `killed`, so the notification reads "Stopped" and names the last agent left in flight). Repeated stops report that a stop is already in flight; stops on a settled run report its status; an unknown id is reported clearly. The tool is registered and hidden in step with `workflowsEnabled`, and is excluded from subagents on the same `SUBAGENT_TOOL_NAMES` boundary.
...
Stop a running or queued subagent by its id (the id returned by the `Agent` tool). The agent is aborted immediately; the call does not wait for it to finish.

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `id` | string | yes | Subagent id to stop |
...
Runtime tuning values set via `/agents` → Settings (max concurrency, max foreground concurrency, default max turns, grace turns, nested depth, fallback agent, default join mode, scheduling on/off, scope models on/off, disable defaults on/off, strict agent files on/off, agent mentions on/off, output transcript on/off, tool description full/compact/custom, widget all/background/off, usage reporting on/off, cost display on/off, model display on/off, viewer markdown off/assistant/all, stuck detection rules/rules+ai + its five knobs) persist across pi restarts. Two files, merged on load:
...
**Stuck detection** (`stuckDetection`, default `"rules"`): detects subagents that are stuck — repeating the identical tool call (same tool + same args), failing in a loop, idling with no output, or having no activity at all. The rule layer runs every `stuckRuleIntervalMs` (default `60000` = 60s); a window that violates marks the run `suspicious`, and after `stuckGraceWindows` (default `3`, ≈180s) consecutive suspicious windows the run is `steer`ed once ("wrap up"). The agent is **not** auto-aborted — the run continues and the notification is delivered. Set `"rules+ai"` to add an AI verdict: when a run stays suspicious, a cheap model (`stuckAiModel`, default `deepseek-v4-flash`, resolved via `resolveModel`) is asked once per `stuckAiIntervalMs` (default `600000` = 10min) whether the run is genuinely stuck — a "not stuck" verdict resets it to healthy, a "stuck" verdict confirms the notification (no auto-abort). The AI prompt carries an activity summary (tool starts/ends, text progress, turn ends) with tool arguments intentionally omitted. Resumed sessions use the same reviewer while the manager retains the session's model registry; if the registry is unavailable, AI review safely degrades to rules-only. Stuck-state callbacks fire only when the reported state changes. All seven fields (including `stuckNotificationTarget`) are settable from `/agents` → Settings and persist in `subagents.json`.

**Stuck notification target** (`stuckNotificationTarget`, default empty = main agent): when set to an agent type (e.g. `"Explore"`) or agent id, `agent_stuck` notifications are steered to that agent's session instead of being delivered to the main agent. Empty or absent means the main agent receives the nudge (the default behavior). Use `stop_subagent` to explicitly stop a stuck agent.
