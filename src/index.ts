  ...
  async function runWorkflowTask(ctx: ExtensionContext, task: WorkflowTask): Promise<void> {
    try {
      const result = await runWorkflow({
        ...
        // P0: the runtime settles through a single exit point, so a hard-killed
        // worker still leaves a terminal record behind — the caller persists it
        // next to the journal (no injection, no write). The terminal notification
        // itself runs once the task record is completed, below.
        ...(task.journalPath !== undefined
          ? {
              persistRunSummary: (summary: WorkflowRunSummary) => {
                try {
                  writeFileSync(
                    task.journalPath!.replace(/\.workflow\.jsonl$/, ".run-status.json"),
                    JSON.stringify(summary, null, 2),
                    "utf-8",
                  );
                } catch (err) {
                  console.warn(
                    `[pi-subagents] could not persist workflow run status: ${err instanceof Error ? err.message : String(err)}`,
                  );
                }
              },
            }
          : {}),
        // Terminal moment: refresh the surfaces immediately. The task-record
        // notification still runs through notifyWorkflowFinished after the
        // settle, which is where the status is complete.
        onTerminal: () => {
          widget.update();
          fleet.update();
        },
        // Stalled is a warning, not a terminal state: no completeWorkflowTask,
        // just a visible nudge.
        onStalled: (info: { idleMs: number }) => {
          widget.update();
          fleet.update();
          scheduleNudge(`stalled-${task.id}`, () => {
            pi.sendMessage<NotificationDetails>({
              customType: "subagent-notification",
              content:
                `Workflow "${task.workflowName ?? task.id}" appears stalled: no agent has reported for ` +
                `${Math.round(info.idleMs / 1000)}s. Open /agents → Workflows to stop or resume it.`,
              display: true,
              details: {
                id: task.id,
