...
- chain(steps: string[]): Promise<{ok: boolean, failedAt?: string}> — run ordered steps STRICTLY SERIALLY: step N+1 starts only after step N completes. The first step whose \`agent()\` returns \`null\` (a failed or skipped child) short-circuits: later steps do not run and the call resolves to \`{ok: false, failedAt: "<step>"\`; a fully successful chain resolves to \`{ok: true}\`. Use it for steps that depend on the previous step's output (diagnose → fix → verify). A fatal run error (cap breach, abort) rethrows so the run fails rather than the chain carrying on. Capped at 4096 steps per call.
