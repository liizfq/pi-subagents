...
/** A watchdog notice: the run is idle but still going. Never a terminal state. */
...
export function collapse(progress: readonly WorkflowEntry[]): CollapsedProgress {
  ...
  for (const entry of progress) {
    ...
    // run_status is neither a phase nor a log — folding it into phaseTitles
    // would create a phase group with an undefined title and corrupt the UI.
    else if (entry.type === "run_status") continue;
