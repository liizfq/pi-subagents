...
/**
 * Reject every pending host call so the script's try/catch/finally can run —
 * a hard terminate() would skip them, which is exactly the bug this fixes.
 * Fatal, so parallel()/pipeline() rethrow instead of folding an abort into an
 * item null and carrying on.
 */
function handleAbortNotice() {
  // Ack first so the host knows this worker is responsive and lets the grace
  // window run; a wedged worker never gets here and is terminated early.
  port.postMessage({ type: "abort-ack" });
  for (const [callId, waiter] of pendingCalls) {
    pendingCalls.delete(callId);
    const error = new Error("Workflow aborted.");
    error.workflowFatal = true;
    waiter.reject(error);
  }
  // An optional script hook gets one chance to tidy up before the grace window
  // closes. It is a plain sandbox global, so scripts assign it: \`__onWorkflowAbort = () => { ... }\`.
  const hook = abortHook;
  if (hook !== null && typeof hook.__onWorkflowAbort === "function") {
    try {
      hook.__onWorkflowAbort();
    } catch {
      // Best-effort: the abort proceeds regardless of what the hook does.
    }
  }
}
...
let nestedCount = 0;
/** The sandbox, so abort-notice can reach the script's optional \`__onWorkflowAbort\`. */
let abortHook = null;
